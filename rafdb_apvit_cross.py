...
def parse_option():
    ...
    if(opt.method == 'SupCon'):
        opt.model_name = '{}_{}_{}_bsz_{}_epoch_{}_ratio_{}_beta_{}_gamma_{}_lr_{}_decay_{}_temp_{}_trial_{}_{}'.\
        format(opt.method, opt.dataset, opt.model, 
               opt.batch_size, opt.epochs, opt.learning_rate, opt.ratio,
               opt.alpha,opt.beta, opt.gamma, 
               opt.learning_rate, opt.weight_decay,
               opt.temp, opt.trial, opt.special )
    else:
        opt.model_name = '{}_{}_{}_bsz_{}_epoch_{}_alpha_{}_beta_{}_gamma_{}_lr_{}_decay_{}_temp_{}_trial_{}_{}'.\
format(opt.method, opt.dataset, opt.model, 
...
def set_model(opt):    
    # 对比预训练
    model = APViT(pret='save/SupCon/rafdb_IR_50_standard_models/SupCon_rafdb_IR_50_bsz_32_epoch_100_ratio_0.001_alpha_0.0_beta_0.001_gamma_0.5_lr_0.001_decay_0.0005_temp_0.07_trial_0_Contra_cosine/cl_rafdb_last.pth')
    
    # 简单预训练
    # model = APViT(pret='save/SupCon/rafdb_IR_50_standard_models/Cross_rafdb_IR_50_bsz_32_epoch_100_alpha_0.0_beta_0.001_gamma_0.5_lr_0.001_decay_0.0001_temp_0.07_trial_0_cosine/cl_rafdb_last.pth')
    # 无预训练
    # model = APViT()
...
def train(train_loader, model, criterion, criterion2, criterion3, optimizer, epoch, opt):
    ...
    for idx, (images, label1s) in enumerate(train_loader):
...
def test(train_loader, model, criterion3, epoch):
    ...
    return correct*100.0/total, losses.avg, label_true, label_pred
...
def draw(opt, y_true, y_pred):    
    ...
    atlas.show()
    atlas.savefig('pic/conf_matrix/{}_{}_{}.png'.format(opt.dataset,opt.model,opt.method))
    atlas.close()
...
def main():
    ...
    with open('datasets/{}/{}_norm_train.pkl'.format(opt.dataset,opt.dataset),'rb') as f:
        train_loader = pickle.load(f)
    
    with open('datasets/{}/{}_norm_test.pkl'.format(opt.dataset,opt.dataset),'rb') as f:
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        # perform test
        if(epoch%5 ==0):
            test_accu , test_loss, true_lb, pred_lb = test(test_loader, model, criterion3, epoch)
    ...
    plt.grid()    
    plt.savefig(os.path.join('pic/{}'.format(opt.dataset),'model_{}_method_{}_beta_{}_alpha_{}_ratio_{}_{}.png'.format(opt.model,opt.method,opt.beta,opt.alpha,opt.ratio,opt.special)))
    plt.close()
    
    accu_data={'epoch':list(range(5,opt.epochs+1,5)),'accu':test_accus, 'loss':test_losses}
    with open(os.path.join('pic/{}'.format(opt.dataset),'model_{}_method_{}_beta_{}_alpha_{}_ratio_{}_{}.pkl'.format(opt.model,opt.method,opt.gamma,opt.beta,opt.alpha,opt.ratio,opt.special)),'wb') as f:
