...
def parse_option():
    ...
    if(opt.method == 'SupCon'):
        ...
        opt.model_name = '{}_{}_{}_bsz_{}_epoch_{}_ratio_{}_alpha_{}_beta_{}_gamma_{}_lr_{}_decay_{}_temp_{}_trial_{}'.\
format(opt.method, opt.dataset, opt.model, 
       opt.batch_size, opt.epochs, opt.ratio,
       opt.alpha,opt.beta, opt.gamma, 
       opt.learning_rate, opt.weight_decay,
...
def train(train_loader, model, criterion, criterion2, criterion3, optimizer, epoch, opt):
    ...
    for idx, (images, label1s) in enumerate(train_loader):
        ...
        loss = Lcro + opt.ratio*(opt.theta*Lsup + opt.beta*Lbt)
...
def main():
    ...
    with open('datasets/{}/{}_norm_train_{}.pkl'.format(opt.dataset,opt.dataset,opt.batch_size),'rb') as f:
        train_loader = pickle.load(f)
    
    with open('datasets/{}/{}_norm_test_{}.pkl'.format(opt.dataset,opt.dataset,opt.batch_size),'rb') as f:
    ...
    plt.grid()    
    plt.savefig(os.path.join('pic/{}'.format(opt.dataset),'model_{}_method_{}_gamma_{}_beta_{}_ratio_{}.png'.format(opt.model,opt.method,opt.gamma,opt.beta,opt.ratio)))

    accu_data={'epoch':list(range(5,opt.epochs+1,5)),'accu':test_accus, 'loss':test_losses}
    with open(os.path.join('pic/{}'.format(opt.dataset),'model_{}_method_{}_gamma_{}_beta_{}_ratio_{}.pkl'.format(opt.model,opt.method,opt.gamma,opt.beta,opt.ratio)),'wb') as f:
