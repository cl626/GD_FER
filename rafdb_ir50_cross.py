...
def parse_option():
    ...
    if(opt.method == 'Cross'):
        opt.model_name = '{}_{}_{}_{}_bsz_{}_epoch_{}_ratio_{}_lr_{}_decay_{}_temp_{}_trial_{}'.\
...
def main():
    ...
    plt.grid()    
    plt.savefig(os.path.join('pic/{}'.format(opt.dataset),'model_{}_method_{}_gamma_{}_beta_{}_alpha_{}_ratio_{}.png'.format(opt.model,opt.method,opt.gamma,opt.beta,opt.alpha,opt.ratio)))
    plt.close()
    
    accu_data={'epoch':list(range(5,opt.epochs+1,5)),'accu':test_accus, 'loss':test_losses}
    
    with open(os.path.join('pic/{}'.format(opt.dataset),'model_{}_method_{}_gamma_{}_beta_{}_alpha_{}_ratio_{}.pkl'.format(opt.model,opt.method,opt.gamma,opt.beta,opt.alpha,opt.ratio)),'wb') as f:
