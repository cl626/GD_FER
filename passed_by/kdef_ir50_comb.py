...
def train(train_loader, model, criterion, criterion2, criterion3, optimizer, epoch, opt):
    ...
    for idx, (images, label1s, label2s) in enumerate(train_loader):
        ...
        loss = Lcro + opt.ratio*(Lsup + opt.gamma*Lview + opt.beta*Lbt)
...
def main():
    ...
    with open('datasets/{ds}/{ds}_norm_train_{bs}.pkl'.format(ds=opt.dataset,bs=opt.batch_size),'rb') as f:
        train_loader = pickle.load(f)
    
    with open('datasets/{ds}/{ds}_norm_test_{bs}.pkl'.format(ds=opt.dataset,bs=opt.batch_size),'rb') as f:
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        # perform test
        if(epoch%5 ==0):
            test_loss, test_acc = test(test_loader, model, criterion3, epoch)
    ...
    plt.grid()    
    plt.savefig(os.path.join('pic/{ds}','{ds}_lr_{lr}.png').format(ds=opt.dataset, lr = opt.learning_rate))
    ...
    with open(os.path.join('pic/{ds}','{ds}_lr_{lr}.pkl').format(ds=opt.dataset, lr = opt.learning_rate),'wb') as f:
              pickle.dump(accu_data,f)
