...
def set_model(opt):    
    # model = IRes_50()
    # model = APViT(pret='save/SupCon/kdef_IR_50_standard_models/SupCon_kdef_IR_50_bsz_256_epoch_500_ratio_0.01_alpha_0.0_beta_0.001_gamma_0.5_lr_0.01_decay_0.0001_bsz_256_temp_0.1_trial_0_cosine/cl_kdef_last.pth',model=opt.model)  # 加预训练
    # model = APViT(model=opt.model)  # 不加预训练
...
def test(train_loader, model, criterion, criterion2, epoch):
    ...
    return losses.avg, correct*100.0/total
...
def main():
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        # perform test
        if(epoch%5 ==0):
            loss , accu = test(test_loader, model, criterion, criterion2, epoch)
    ...
    plt.grid()    
    # plt.savefig(os.path.join('pic/apvit/','apvit_entire_lr_{}_wd_{}_{}.png'.format(opt.learning_rate,opt.weight_decay,opt.special)))

    accu_data={'epoch':list(range(5,opt.epochs+1,5)),'accu':test_accus, 'loss':test_losses} 
    # with open(os.path.join('pic/apvit','apvit_entire_lr_{}_wd_{}_{}.pkl'.format(opt.learning_rate,opt.weight_decay,opt.special)),'wb') as f:
