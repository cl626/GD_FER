...
def parse_option():
    ...
    opt.model_name = 'label2__{}_{}_{}_resnet50_{}_alpha_{}_bigbeta_{}_gamma_{}_bsz_{}_temp_{}_trial_{}'.\
        format(opt.method, opt.dataset, opt.model, opt.resnet50,
               opt.alpha,opt.beta, opt.gamma, 
               opt.batch_size, opt.temp, opt.trial )
    # opt.model_name = '{}3_{}_{}_{}_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_trial_{}'.\
    #     format(opt.method, opt.dataset, opt.model, opt.alpha,
    #            opt.beta, opt.gamma, opt.learning_rate,
...
class PoseDataset(data.Dataset):
    def __init__(self, path, path2):
        ...
        root_dir2 = os.path.join(os.getcwd(),path2)
        dir_list2 = os.listdir(root_dir2)
        self.data = []
        ...
        for dir_name in dir_list2:
            tmp_dir = os.path.join(os.path.join(root_dir2,dir_name))
            img_list = os.listdir(tmp_dir)
            for img_name in img_list:
                t_img = self.mytransform(Image.open(os.path.join(tmp_dir,img_name)).convert("RGB"))
                print('\r{},{},{}'.format(img_name,type(t_img),t_img[0].shape),end='')
...
def set_loader_kdef(opt):
    # mydataset = PoseDataset('./datasets/KDEF/AF','./datasets/KDEF/BF')
    with open('kdef_label2.pkl','rb') as f:
        mydataset = pickle.load(f)
    # with open("kdef_label2.pkl",'wb') as f:
...
def train(train_loader, model, criterion, criterion2, optimizer, epoch, opt):
    """one epoch training"""
    model.train()
    print(opt.alpha,opt.beta,opt.gamma)
    ...
    for idx, (images, label1s, label2s) in enumerate(train_loader):
        ...
        # compute loss
        # print("labels={},images={}".format(labels,images))
        # print("images2.shape={}".format(images.shape))
        mid1 , mid2 , res = model(images)
        # print("shape of mid1={}, mid2={}, label1s={}, label2s={}".format(mid1.shape,mid2.shape,label1s.shape,label2s.shape))
        features = nn.functional.normalize(res,dim=1)
        # print("features' shape = {}".format(features.shape))
        f1, f2 = torch.split(features, [bsz, bsz], dim=0)
        # print("f1' shape = {}".format(f2.shape))
        # print("f2' shape = {}".format(f2.shape))
        features = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)
        # print("then , features' shape = {}".format(features.shape))

        # print("features={},shape={}".format(features,features.shape))
        if opt.method == 'SupCon':
            # loss = criterion(features, label1s)
        ...
        # print((mid1[:8]).shape,(mid1[8:]).shape)
        # import pdb; pdb.set_trace()
        # print(loss)
        Lbt = criterion2(mid1[:opt.batch_size], mid1[opt.batch_size:])+criterion2(mid2[:opt.batch_size], mid2[opt.batch_size:])
        
        # print('Lsup={:6f}\t Lview={:6f}\t Lbt={:6f}'.format(Lsup,Lview,Lbt))
        loss = Lsup + opt.gamma*Lview + opt.beta*Lbt
        ...
        if (idx + 1) % opt.print_freq == 0:
            print('Train: [{0}][{1}/{2}]\t'
                  'Lsup={3:6f}\t Lview={4:6f}\t Lbt={5:6f}\t'
                  'loss {loss.val:.3f} ({loss.avg:.3f})'.format(
                   epoch, idx + 1, len(train_loader), Lsup, Lview, Lbt, loss=losses))
            sys.stdout.flush()
...
def main():
    ...
    train_loader = set_loader_kdef(opt)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if epoch % opt.save_freq == 0:
            save_file = os.path.join(
                opt.save_folder, 'cl_kdef_epoch_{epoch}.pth'.format(epoch=epoch))
