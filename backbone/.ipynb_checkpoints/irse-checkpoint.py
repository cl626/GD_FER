...
class IRSE(BaseBackbone):
    ...
    def attention_map(self,x):
        attn_map = lambda x: torch.sum(torch.abs(x), dim=1).unsqueeze(1)
        y = attn_map(x).flatten(2)
        # print(y.shape)

        _,keep_index = torch.topk(y,k=32,largest=False)
        mask = torch.tensor([False]*x.size(0)*x.size(2)*x.size(3))
        mask = torch.reshape(mask,(x.size(0),x.size(2),x.size(3))).cuda()

        # print(keep_index.shape)
        # print(keep_index[0][0])
        for set in range(keep_index.size(0)):
            for idx in range(keep_index.size(2)):
                t = keep_index[set][0][idx]
                x[set,:,t//14,t%14]= 0
    ...
    def forward(self, x):
        ...
        for index, m in enumerate(self.body):
            x = m(x)
            # if index ==2:   
            #     x = self.attention_map(x)  # 用于Iresnet中添加attn_map
